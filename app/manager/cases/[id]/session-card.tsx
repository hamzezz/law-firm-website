...
export default function ManagerSessionCard({ session, caseId, caseNumber, clientName, uploadedBy, sessionDocs, isFuture }: any) {
  ...
  if (isFuture) {
    return (
      <div className="relative pr-10">
        <div className="absolute right-0 top-1.5 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 border-white shadow bg-slate-200 text-slate-400">🔒</div>
        <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 opacity-70">
          <div className="flex items-center justify-between mb-1">
            <h3 className="font-bold text-sm text-slate-500">الجلسة رقم {session.session_order}</h3>
            <span className="text-[11px] text-slate-400">{sessionStatusLabel(session.status)}</span>
          </div>
          <p className="text-xs text-slate-400">موعدها {sessionDateFormatted} - لم تنعقد بعد</p>
        </div>
      </div>
    )
  }
  ...
  if (locked) {
    return (
      <div className="relative pr-10">
        <div className="absolute right-0 top-1.5 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 border-white shadow bg-amber-500 text-white">{session.session_order}</div>
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-bold text-slate-900">الجلسة رقم {session.session_order}</h3>
            <span className="text-[11px] bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full font-bold">{sessionStatusLabel(session.status)}</span>
          </div>
          <p className="text-xs text-slate-400 mb-3">{sessionDateFormatted}</p>
          ...
          {session.next_session_date && (
            <p className="text-xs text-amber-700 font-bold mb-3">الجلسة القادمة: {new Date(session.next_session_date).toLocaleDateString('ar-SA')}</p>
          )}
          ...
          {sessionDocs.length > 0 && (
            <div className="space-y-1.5 pt-2 border-t border-slate-100">
              ...
              {sessionDocs.map(function (d: any) {
                ...
                return (
                  <a key={d.id} href={docUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-xs text-amber-700 hover:text-amber-900 hover:underline transition">
                    <span>📄</span>
          ...
          <p className="text-[10px] text-slate-400 mt-3">بيانات هذه الجلسة نهائية ولا يمكن تعديلها</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="relative pr-10">
      <div className="absolute right-0 top-1.5 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 border-white shadow bg-amber-400 text-white animate-pulse">{session.session_order}</div>
      <div className="bg-amber-50/50 rounded-2xl border border-amber-200 p-5">
        <div className="flex items-center justify-between mb-2">
          <h3 className="font-bold text-slate-900">الجلسة رقم {session.session_order}</h3>
          <span className="text-[11px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-bold">{sessionStatusLabel(session.status)}</span>
        </div>
        <p className="text-xs text-slate-400 mb-3">{sessionDateFormatted}</p>
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2 items-center flex-wrap">
            ...
            <input type="date" value={nextDate} onChange={function (e) { setNextDate(e.target.value) }} disabled={saving} className="px-3 py-1 border border-amber-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-amber-500 bg-white" />
          </div>
